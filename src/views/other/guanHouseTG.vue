<template>
	<div class="bjs" @click="downApp">
		<div class="bannerList">
			<img v-lazy="topBanner" />
			<div>
				<a href="javascript:;" class="yellowBtn">活动需知</a>
				<a href="javascript:;" class="lightYellowBtn">关于冠寓</a>				
			</div>
		</div>
		<table class="cityList" >
			<tr>
				...
				<th><a href="javascript:;"  id="020" @click="code22()">广州</a></th>
		...
		<div class="wm30">
			<div class="activeList" v-if="areaCode == '021'">					
				<a href="javascript:;" :lang = 'it' :tabIndex="key" v-for="(it,key) in sh" :key="key" v-if="it.length!=1"  :class="tempArr[key]?'cur':''" id="sh">{{it}}</a>												
			</div>
			<div class="activeList" v-else-if="areaCode == '010'">
				<a href="javascript:;" :lang = 'it'  :tabIndex="key" v-for="(it,key) in bj" :key="key" v-if="it.length!=1" :class="tempArr[key]?'cur':''"  id="bj">{{it}}</a>												
			</div>
			<div class="activeList" v-else-if="areaCode == '020'">					
				<a href="javascript:;" :lang = 'it'  :tabIndex="key" v-for="(it,key) in gz" :key="key" v-if="it.length!=1"  :class="tempArr[key]?'cur':''" id="gz">{{it}}</a>												
			</div>
			<div class="activeList" v-else-if="areaCode == '0755'" > 
				<a href="javascript:;" :lang = 'it' :tabIndex="key" v-for="(it,key) in sz" :key="key" v-if="it.length!=1" :class="tempArr[key]?'cur':''"  id="sz">{{it}}</a>												
			</div>
			<div class="activeList cds" v-else-if="areaCode == '028'">					
				<a href="javascript:;" :lang = 'it' :tabIndex="key" v-for="(it,key) in cd" :key="key" v-if="it.length!=1"  :class="tempArr[key]?'cur':''" id="cd">{{it}}</a>												
			</div>
			<div class="activeList" v-else-if="areaCode == '0571'">
				<a href="javascript:;" :lang = 'it' :tabIndex="key" v-for="(it,key) in hz" :key="key" v-if="it.length!=1" :class="tempArr[key]?'cur':''"  id="hz">{{it}}</a>												
			</div>
		</div>
		<div class="houseListPd">
			...
			<div class="tanBg tanBg01">
				<div class="tanBgPd">
					<a href="javascript:;" ><img src="https://opres.gongyujia.com/H5/guanHouse/close.png"></a>
					<h4>活动需知</h4>
		           ...
		           if(navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
		                ...
		                window.setTimeout(function() {
	                        ...
	                        if(timeOutDateTime - loadDateTime < 5000) {
	                            window.location = "https://at.umeng.com/HDiaym";
	                        } else {
	                            window.close();
	                        }
	                    },25);
		        	...
		        	if(navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
		                ...
		                window.setTimeout(function() {
	                        var timeOutDateTime = new Date();
	                        if(timeOutDateTime - loadDateTime < 5000) {		                        	
	                            window.location.href = "https://at.umeng.com/HDiaym";
	                        } else {
	                            window.close();
	                        }
	                    },25);			               
